...
public class SqlSessionFactoryBean implements FactoryBean<SqlSessionFactory>, InitializingBean, ApplicationListener<ApplicationEvent>,ApplicationContextAware {
    ...
    //更改 Search 
    private static final String searchPath = "com.pandawork.core.search.cfg.ConfigBean";
    ...
    protected SqlSessionFactory buildSqlSessionFactory() throws IOException {
        ...
        // modify by lionel 支持文件夹扫描
        // 扫描文件夹定义文件
        Resource[] newMapperLocations = null;
        mapperFolder += ";com.pandawork.core.dao.impl.sql"; // 添加core中的commondao的mapper文件
        if(!ObjectUtils.isEmpty(mapperLocations) && !"".equals(mapperFolder)){
            Resource[] folderMappers = MapperUtil.mutilScan(mapperFolder);
            int newLength = mapperFolder.length() + mapperLocations.length;
            newMapperLocations = new Resource[newLength];
            System.arraycopy(mapperLocations, 0, newMapperLocations, 0, mapperLocations.length);
            System.arraycopy(folderMappers, 0, newMapperLocations, mapperLocations.length, mapperLocations.length);
        } else if(!ObjectUtils.isEmpty(mapperLocations)){
            newMapperLocations = mapperLocations;
        } else if(!"".equals(mapperFolder)){
            newMapperLocations = MapperUtil.mutilScan(mapperFolder);
        }
        
        if (!ObjectUtils.isEmpty(newMapperLocations)) {
            for (Resource mapperLocation : newMapperLocations) {
        ...
        //启动Search
        if(enableSearch){
        	ConfigBean cfg = this.context.getBean(ConfigBean.class);
        	if(cfg == null){
        		throw new IllegalArgumentException("ConfigBean 没有正确配置");
        	}
        	cfg.initIbatisSearch();
        }
