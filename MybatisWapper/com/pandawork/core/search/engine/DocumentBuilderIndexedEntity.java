...
public class DocumentBuilderIndexedEntity<T> extends AbstractDocumentBuilder<T> {
	...
	public DocumentBuilderIndexedEntity(Class<?> beanClass,ConfigBean configBean) {
		super(beanClass, configBean, configBean.getDefaultSimilarity());
		this.directoryProviders = configBean.getDirectory();
	}
	
	protected void documentBuilderSpecificChecks(Member member, PropertyMeta propertiesMetadata, ConfigBean context) {
		checkDocumentId( member, propertiesMetadata ,context );
	}
	
	protected void checkDocumentId(Member member,PropertyMeta meta,ConfigBean context){
		...
		if(idAnnotation != null){
			...
			FieldBridge fieldBridge = BridgeFactory.guessType( null, numericFieldAnn, member );
	...
	public <T> Document getDocument(T entity, Serializable id, Map<String, String> fieldToAnalyzerMap) {
...
//		);
		...
		buildDocumentFields( entity, doc, getMetadata(), fieldToAnalyzerMap, processedFieldNames);
		return doc;
	}
	
	public Term getTerm(Serializable id) {
		return new Term( identifierName, objectToString( idBridge, identifierName, id ) );
	}
	...
	private void buildDocumentFields(Object instance, Document doc,
		...
		for (int i = 0; i < propertiesMetadata.fieldNames.size(); i++) {
			...
			Object value = com.pandawork.core.util.ReflectionHelper.getMemberValue(instance,member);
			...
			Object memberValue = ReflectionHelper.getMemberValue(instance, member);
			String valueString = ((TwoWayString2FieldBridgeAdaptor)fieldBridge).objectToString(memberValue);
			LuceneOptions opts = propertiesMetadata.getClassLuceneOptions(i);
	...
	private Annotation getIdAnnotation(Member member, ConfigBean context) {
		...
		DocumentId documentIdAnn = ((AnnotatedElement)member).getAnnotation(DocumentId.class);
